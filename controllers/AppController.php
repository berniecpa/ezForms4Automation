...
class AppController extends Controller
{
    ...
    public function actionForm($id, $sid = 0, $t = 1, $b = 1, $js = 1, $rec = 1, $p = 1, $r = 1)
    {
        ...
        $settingsForm = new FormPageSettingsForm;
        ...
        // Default values
        $settingsForm->favicon = $formModel->ui->favicon;
        $settingsForm->logo = $formModel->ui->logo;
        $settingsForm->logo_link = $formModel->ui->logo_link;
        $settingsForm->logo_link_url = $formModel->ui->logo_link_url;
        $settingsForm->styles = $formModel->ui->styles;
        $settingsForm->meta_title = $formModel->ui->meta_title;
        $settingsForm->meta_description = $formModel->ui->meta_description;
        $settingsForm->meta_image = $formModel->ui->meta_image;

        // Save page settings
        if ($settingsForm->load($postData)) {
            $settingsForm->favicon = UploadedFile::getInstance($settingsForm, 'favicon');
            $settingsForm->logo = UploadedFile::getInstance($settingsForm, 'logo');
            $settingsForm->meta_image = UploadedFile::getInstance($settingsForm, 'meta_image');
            $settingsForm->form_id = $formModel->id;
    ...
    public function actionEmbed($id, $sid = 0, $t = 1, $js = 1, $rec = 1, $p = 1, $r = 1)
    {
        ...
        if ($formModel->status === Form::STATUS_INACTIVE) {
            return $this->render('message', [
                'formModel' => $formModel,
                'messageType' => Form::INACTIVE_MESSAGE_TYPE,
            ]);
        }

        // Display Message when URL is unauthorized
        if ($formModel->authorized_urls === Form::ON
            && $formModel->authorized_urls_error_type === Form::ON) {

            if (!$formModel->isInAuthorizedUrl()) {
                return $this->render('message', [
                    'formModel' => $formModel,
                    'messageType' => Form::UNAUTHORIZED_MESSAGE_TYPE,
                ]);
            }
        }

        // Restrict access when form is password protected
        if ($formModel->use_password === Form::ON) {
            ...
            if (!empty($post)) {
                ...
                if (!$formModel->isInAuthorizedUrl()) {
                    $message = !empty($formModel->authorized_urls_error_message)
                        ...
                        : Yii::t('app', 'You are not authorized to access this form.');
                ...
                if ($response['success'] && $formSubmissionModel->load($postFormSubmission) && $formSubmissionModel->validate()) {
                    ...
                    if (!empty($formConfirmationModel->message)) {
                        ...
                        $response['confirmationCustomHtml'] = $formConfirmationModel->custom_html;
                    }
                    ...
                    if (count($formModel->formConfirmationRules) > 0) {
                        ...
                        foreach ($formModel->formConfirmationRules as $ruleModel) {
                            ...
                            if ($isValid) {
                                ...
                                $response['confirmationCustomHtml'] = $ruleModel->custom_html;
    ...
    public function actionC($s)
    {
        ...
        $sid = is_numeric($s) ? $s : Hashids::decode($s);
        ...
        $message = Yii::t('app', 'Your Form Submission has been confirmed previously. {startTag}Thank you!{endTag}.', [
            'startTag' => '<strong>',
            'endTag' => '</strong>',
        ]);
        $default = true;
