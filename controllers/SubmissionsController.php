...
class SubmissionsController extends ActiveController
{
    ...
    public function actions()
    {
        ...
        $actions['index']['prepareDataProvider'] = function () {
            ...
            if ($formModel) {
                // Time Zone
                if (!empty($formModel->submission_timezone)) {
                    Yii::$app->timeZone = $formModel->submission_timezone;
                }
                // Date Format
                if (!empty($formModel->submission_dateformat)) {
                    /** @var \kartik\datecontrol\Module $dateControlModule */
                    $dateControlModule = Yii::$app->getModule('datecontrol');
                    $dateControlModule->displaySettings['datetime'] = $formModel->submission_dateformat;
                }
            }

            if (!empty($q)) {
                $escape = [];
                if ($formModel->language !== 'ar-EG') { // This is incompatible with Arabic language
            ...
            if (!Yii::$app->user->can('configureForms', ['model' => $formModel])) {
                if ($formModel->submission_scope === Form::SUBMISSION_SCOPE_OWNER) {
