...
(function ($) {
    ...
    Api.prototype = {
        ...
        runOne: function(base) {
            ...
            var initialValue = base.find(this.details.resultSelector).val();
            var formattedResult = this.opts.outputFormatter(result);
            if (initialValue != formattedResult) {
                var el = base.find(this.details.resultSelector);
                el.val(formattedResult);
                setTimeout(function () {el.trigger('input') }, 0);
            }
        }
    }
