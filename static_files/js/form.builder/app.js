...
define([
    ...
    , "helper/pubsub"
    , "jquery.fontselect", "jquery.flexdatalist", "jquery.cookie", "spectrum", "spectrum-kv"
], function(
    $, _, Backbone, Polyglot
                ...
                $.ajax(options.initPoint)
                    ...
                    if (!_.isUndefined(initForm['styles'])) {
                        cssRules = _.map(cssRules, function (defaultRule) {
                            ...
                            if (typeof cssRule === 'object' && cssRule.hasOwnProperty('selector')) {
                                delete cssRule.selector;
                            }
            ...
            $('#actions').find('.saveForm').click(function( e ){
                ...
                $.ajax({
                    ...
                    if (data.success && data.id > 0) {
                        ...
                        let messageBody = typeof data.message !== 'undefined'
                            ? data.message
                            : polyglot.t('alert.errorSavingData');
                        $('#toast-body-status').html(polyglot.t('alert.warning'));
                        $('#toast-body-message').html(messageBody);
                    ...
                    const toastEl = document.getElementById('toast')
                    $('#toast-body-status').html(polyglot.t('alert.warning'));
                    $('#toast-body-message').html(polyglot.t('alert.errorSavingData'));
                    const toast = new bootstrap.Toast(toastEl)
                    toast.show();
