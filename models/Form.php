 ...
 * @property integer $authorized_urls_error_type
 * @property string $authorized_urls_error_message
...
class Form extends ActiveRecord
{
    ...
    const INACTIVE_MESSAGE_TYPE = 'INACTIVE';
    const UNAUTHORIZED_MESSAGE_TYPE = 'UNAUTHORIZED';
    ...
    public function rules()
    {
        return [
            ...
            [['message', 'authorized_urls_error_message', 'submission_editable_conditions', 'preferences'], 'string'],
            [['status', 'is_private', 'use_password', 'authorized_urls', 'authorized_urls_error_type', 'schedule', 'schedule_start_date', 'schedule_end_date',
    ...
    public function attributeLabels()
    {
        return [
            ...
            'urls' => Yii::t('app', 'URLs'),
            'authorized_urls_error_type' => Yii::t('app', 'Hide Form'),
            'authorized_urls_error_message' => Yii::t('app', 'Unauthorized Url Message'),
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if ($insert) {
            // Create protected files directory, if it doesn't exist
            ...
            $publicFilesDirectory = $this->getPublicFilesDirectory();
            if (!is_dir($publicFilesDirectory)) {
                Yii::$app->fs->createDir($publicFilesDirectory);
            }
        }
    }
    ...
    /**
     * Check if referrer page is an authorized host
     *
     * @throws NotFoundHttpException
     */
    public function isInAuthorizedUrl()
    {
        if ($this->authorized_urls === Form::OFF || empty(trim($this->urls))) {
            return true;
        }
        ...
        if (!empty($_SERVER['HTTP_REFERER'])) {
            ...
            // If referrer is in an authorized host
    ...
    {
        return 'app/' . FormSubmissionFile::FILES_FOLDER . '/' . $this->hashId;
    }

    public function getSignaturesDirectory(): string
    {
        return FormSubmissionFile::FILES_FOLDER . '/signatures';
    }

    public function getSignaturesDirectoryAlias()
    {
        return Yii::getAlias('@uploads/' . $this->getSignaturesDirectory());
    }
}
